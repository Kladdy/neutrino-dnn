...
if fit:
    ...
    popt, pcov = curve_fit(f, xdata, ydata, p0=[100000, 1])
...
if eps:
    plt.savefig(f"{plots_dir}/angular_resolution_{run_name}_Rayleigh_fit.eps", format="eps")
else:  
    plt.savefig(f"{plots_dir}/angular_resolution_{run_name}_Rayleigh_fit.png")
