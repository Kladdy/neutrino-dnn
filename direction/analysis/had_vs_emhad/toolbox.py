...
def load_file(i_file, dataset, plots_dir, norm=1e-6):
    # Load 500 MHz filter
...
def load_file_all_properties(i_file, dataset, plots_dir, norm=1e-6):
    t0 = time.time()
    print(f"loading file {i_file}", flush=True)
...
def get_pred_angle_diff_data(run_name, dataset, plots_dir, prediction_file):
...
def get_pred_angle_diff_data_and_angles(run_name, dataset, plots_dir, prediction_file):
    with open(prediction_file, "br") as fin:
        nu_direction_predict, nu_direction = pickle.load(fin)
